...
**excluded**: Numeric vector of length 2: `c(post, pre)`.  `post` = number of cutoff‐dates *after* the main one to skip; `pre` = number of cutoff‐dates *before* the main one to skip. Defaults to `c(Inf, 0)`, i.e. exclude all post‑cutoff placebos and none pre‑cutoff
...
**excluded**: Numeric vector of length 2, `c(post_excl, pre_excl)`. Defaults to `c(Inf, 0)`, i.e. exclude **all** post‐cutoff placebo dates, none pre‐cutoff.
